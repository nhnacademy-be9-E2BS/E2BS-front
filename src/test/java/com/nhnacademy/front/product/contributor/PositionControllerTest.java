...
class PositionControllerTest {
	...
	void createPositionSuccess() throws Exception {
		// given & when & then
		mockMvc.perform(post("/admin/settings/positions")
				.param("positionName", "작가")
				.with(csrf()))
			.andExpect(status().is3xxRedirection())
			.andExpect(redirectedUrl("/admin/settings/positions"));
	}
	...
	void updatePositionSuccess() throws Exception {
		mockMvc.perform(put("/admin/settings/positions/1")
	...
	void getPositionSuccess() throws Exception {
		...
		mockMvc.perform(get("/admin/settings/positions/1")
				.with(csrf()))
	...
	void getPositionsSuccess() throws Exception {
		...
		mockMvc.perform(get("/admin/settings/positions"))
			.andExpect(status().isOk())
			.andExpect(view().name("admin/product/positions"))
