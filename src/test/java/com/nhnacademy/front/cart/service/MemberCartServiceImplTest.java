...
class MemberCartServiceImplTest {
	...
	void deleteCartItemForMember() {
		// given
		RequestDeleteCartItemsForMemberDTO dto = new RequestDeleteCartItemsForMemberDTO();
		when(memberCartAdaptor.deleteCartItemForMember(dto)).thenReturn(ResponseEntity.noContent().build());

		// when & then
		assertThatCode(() -> memberCartService.deleteCartItemForMember(dto))
			.doesNotThrowAnyException();
	}
	...
	void deleteCartItemForMember_Fail_StatusNot2xx() {
		...
		when(memberCartAdaptor.deleteCartItemForMember(dto)).thenReturn(response);

		// when & then
		assertThatThrownBy(() -> memberCartService.deleteCartItemForMember(dto))
			.isInstanceOf(CartProcessException.class)
	...
	void deleteCartItemForMember_Fail_FeignException() {
		// given
		RequestDeleteCartItemsForMemberDTO dto = new RequestDeleteCartItemsForMemberDTO();
		when(memberCartAdaptor.deleteCartItemForMember(dto)).thenThrow(FeignException.class);
